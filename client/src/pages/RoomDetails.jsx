...
const RoomDetails = () => {
  ...
  //Check if the Room is Avialable
  const checkAvailablilty = async ()=>{
    try {
  ...
  const onSubmitHandler = async (e)=>{
    try {
        e.preventDefault();
        if (!isAvailable) {
            return checkAvailablilty();
        }else{
            const { data } = await axios.post('/api/bookings/book', {room: id, checkInDate, checkOutDate, guests, paymentMethod: "Pay At Home"}, {headers: { Authorization: `Bearer ${await getToken()}`}})
            if (data.success) {
      ...
      <div className="py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32">
        ...
        <div className="max-w-3x1 border-y border-gray-300 my-15 py-10 text-gray-500">
          <p>
            ...
            availability. You get a comfortable two bed apartment has a true
            city feeling. The price quoted is for two guest, at the guest slot
            please remark the number of guests to the to get the exact price for
            groups.
          </p>
        </div>
        ...
        <div className="flex flex-col items-start gap-4">
          ...
          <button className="px-6 py-2.5 mt-4 rounded text-white bg-primary hover:bg-primary-dull transition-all cursor-pointer font-playfair">
            Contact Now
          </button>
        </div>
      </div>
